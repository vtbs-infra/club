// @ts-expect-error The package publishes index.d.ts but omits it from its ESM exports map.
import ExcelJsUntyped from '@excel.js/exceljs';
import type { Cell, Workbook } from '../../../../node_modules/@excel.js/exceljs/index.js';
...
const ExcelJS = ExcelJsUntyped as unknown as {
...
export async function buildGuardAddressWorkbook(input: GuardAddressWorkbookInput): Promise<Buffer> {
  ...
  for (const [index, item] of input.rows.entries()) {
    ...
    const row = worksheet.addRow([
      ...
      ORDER_STATUS_LABELS[item.orderStatus] ?? safeSpreadsheetText(item.orderStatus),
    ]);
