...
const CommonStyleSchema = Type.Object(
  {
    ...
    backgroundAssetId: Type.Optional(Type.String({ format: 'uuid' })),
  ...
  Type.Object(
    {
      ...
      backgroundDesktopAssetId: Type.Optional(Type.String({ format: 'uuid' })),
      backgroundMobileAssetId: Type.Optional(Type.String({ format: 'uuid' })),
      description: Type.String({ maxLength: 800 }),
      eyebrow: Type.String({ maxLength: 120 }),
      primaryAction: Type.Optional(ActionSchema),
      secondaryAction: Type.Optional(ActionSchema),
...
const RichTextBlockSchema = block(
  ...
  Type.Object(
    {
      actions: Type.Array(ActionSchema, { maxItems: 4 }),
...
export type SiteAction = Static<typeof ActionSchema>;
...
export function isSitePageContent(value: unknown): value is SitePageContent {
  ...
  for (const currentBlock of content.blocks) {
  ...
  blocks: [
    {
      ...
      content: {
        ...
        eyebrow: '舰长礼物计划',
        primaryAction: { href: '/claims', label: '查看我的礼物' },
