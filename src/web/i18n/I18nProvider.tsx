...
const englishToChinese = Object.fromEntries(
  Object.entries(chineseToEnglish).map(([chinese, english]) => [english, chinese]),
...
function localizedValue(value: string, language: Language): string {
  ...
  const translated = language === 'en' ? chineseToEnglish[content] : englishToChinese[content];
...
export function I18nProvider({ children }: { readonly children: ReactNode }) {
  ...
  const setLanguage = useCallback((nextLanguage: Language) => {
    ...
    try {
      ...
      // Switching still works when persistent storage is unavailable.
    }
  }, []);
  ...
  const t = useCallback(
    (chinese: string, english?: string) =>
      language === 'zh-CN' ? chinese : (english ?? chineseToEnglish[chinese] ?? chinese),
    [language],
  );
