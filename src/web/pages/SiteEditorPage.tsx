...
type EditorSection = 'site' | 'content' | 'assets';
...
function SiteEditor({ identity }: { readonly identity: Identity }) {
  const client = useQueryClient();
  const admin = useQuery({ queryFn: getHomepageAdmin, queryKey: ['platform', 'site', 'home'] });
  const preview = useQuery({
    queryFn: getHomepagePreview,
    queryKey: ['platform', 'site', 'home', 'preview'],
    retry: false,
  });
  ...
  const [selectedId, setSelectedId] = useState<string | null>(
    defaultSitePageContent.blocks[0]?.id ?? null,
  );
  ...
  const [loadedDraftId, setLoadedDraftId] = useState<string | null | undefined>(undefined);

  const synchronize = (state: SiteAdminState) => {
    client.setQueryData(['platform', 'site', 'home'], state);
    setContent(structuredClone(state.draft.content));
    setSelectedId((current) =>
      state.draft.content.blocks.some((block) => block.id === current)
        ? current
        : (state.draft.content.blocks[0]?.id ?? null),
    );
  };
  if (admin.data && loadedDraftId !== admin.data.draft.id) {
    setLoadedDraftId(admin.data.draft.id);
    setContent(structuredClone(admin.data.draft.content));
    setSelectedId((current) =>
      admin.data.draft.content.blocks.some((block) => block.id === current)
        ? current
        : (admin.data.draft.content.blocks[0]?.id ?? null),
    );
  }

  const savedContent = admin.data?.draft.content ?? defaultSitePageContent;
  const dirty = JSON.stringify(content) !== JSON.stringify(savedContent);
  const valid = isSitePageContent(content);
  const save = useMutation({
    mutationFn: () => saveHomepageDraft(content, admin.data?.draft.id ?? null),
    onSuccess: synchronize,
  });
  const publish = useMutation({
    mutationFn: () => {
      if (!admin.data?.draft.id) throw new Error('请先保存草稿。');
      return publishHomepage(admin.data.draft.id);
    },
    onSuccess: async (state) => {
      synchronize(state);
      await Promise.all([
        client.invalidateQueries({ queryKey: ['site', 'home'] }),
        client.invalidateQueries({ queryKey: ['platform', 'site', 'home', 'preview'] }),
      ]);
    },
  });
  const restore = useMutation({
    mutationFn: (versionId: string) =>
      restoreHomepageVersion(versionId, admin.data?.draft.id ?? null),
    onSuccess: synchronize,
  });
  const error = admin.error ?? save.error ?? publish.error ?? restore.error;
  ...
  return (
    <section className="site-editor-page">
      <header className="site-editor-toolbar">
        <div>
          <p className="section-kicker">站点内容</p>
          <h1>首页编辑器</h1>
          <span>
            {dirty ? '有未保存修改' : '草稿已保存'} · 已发布版本{' '}
            {admin.data?.published.version ?? 0}
          </span>
        </div>
        <div className="site-editor-toolbar-actions">
          <button
            className={`button button-secondary ${previewMode === 'desktop' ? 'is-active' : ''}`}
          ...
          <button
            className={`button button-secondary ${previewMode === 'mobile' ? 'is-active' : ''}`}
          ...
          <button
            className="button"
            disabled={dirty || !admin.data?.draft.id || publish.isPending}
      ...
      <div className="site-editor-navigation">
        <button
        ...
        <Link to="/platform/appearance">主题与外观</Link>
      </div>

      {section === 'site' ? (
        <section className="site-information-panel">
          <div>
      ...
      {section === 'content' ? (
        <div className="site-editor-workspace">
          <aside className="site-editor-blocks">
            ...
            <details className="site-version-history">
              ...
              {admin.data?.versions.map((version) => (
                <div key={version.id}>
                  ...
                  <button
                    disabled={dirty || restore.isPending || version.id === admin.data?.draft.id}
