...
本文档集描述当前源码基线的产品行为、部署方式和维护边界。

当前用户名认证基线是未发布的不兼容变更，要求新部署；旧 v0.2.0 镜像和数据库不适用。
...
| 注册、会话与密码找回         | [账号认证](authentication.md)       |
...
当前源码基线支持一个 Club 应用实例、一个 PostgreSQL 17 数据库和一个私有本地存储卷。
