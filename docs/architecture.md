...
公告的内容、发布、置顶和可见性修改都在事务内递增版本并写入审计。`announcement_reads`
记录用户实际读到的版本，而不是永久布尔值；公告更新后，旧版本的已读记录不会遮蔽新
内容。平台公告和主播公告共享同一生命周期，接收范围由查询服务按账号和礼物单关系决定。
