...
- [x] 确定最终令牌契约；
- [x] 将承担语义的色相变量替换为用途变量；
- [x] 将基础、Shell、用户和管理样式中的可见颜色收敛到令牌；
- [x] 把公开首页与认证样式拆为 `portal.css` 和 `auth.css`；
- [x] 保持默认 Moe 外观、布局和交互行为不变；
- [x] 完成源码审查、`pnpm check`、单元测试和 Web 构建。

### 阶段二：平台外观领域

- [x] 增加共享 TypeBox 契约；
- [x] 增加 Drizzle Schema、SQL 迁移、约束和默认行；
- [x] 实现 `AppearanceService` 的读取、更新、事务和审计；
- [x] 注册公开读取与管理员更新路由；
- [x] 验证 OpenAPI；
- [x] 完成 PostgreSQL 集成与迁移测试。

### 阶段三：全局主题运行时

- [x] 增加 Web API Client 与穷尽主题定义；
- [x] 增加 `AppearanceProvider` 和预览状态；
- [x] 设置初始 HTML、`data-app-theme`、`theme-color` 与 `color-scheme`；
- [x] 完成 Moe、Neon、Archive 和 Pixel 令牌；
- [x] 适配公共、认证、用户、主播、管理员与 Radix Portal；
- [x] 删除遗留硬编码颜色和组件级主题特殊分支。

### 阶段四：管理员主题管理

- [x] 增加 `/admin/appearance` 懒加载路由与导航；
- [x] 实现当前主题、四套卡片和代表色；
- [x] 实现本地整页预览、取消、卸载恢复和显式应用；
- [x] 复用现有加载、错误和操作反馈组件；
- [x] 完成主题管理浏览器测试。
...
- [x] 按代表页面矩阵检查四套主题；
- [x] 修复对比度、溢出、状态和浮层问题；
- [x] 运行格式、Lint、TypeScript、单元、集成、构建和浏览器测试；
- [x] 更新架构、产品指南与管理员文档，只描述最终行为；
- [x] 构建 Docker 镜像、应用迁移并重新部署开发环境；
- [x] 验证 readiness、公开主题和管理员应用流程。
