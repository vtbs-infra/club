...
CREATE TABLE "users" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "users_id_uid_unique" UNIQUE("id","bilibili_uid"),
	CONSTRAINT "users_username_check" CHECK ("users"."username" ~ '^[a-z0-9_]{3,30}$'),
	CONSTRAINT "users_role_check" CHECK ("users"."role" in ('USER', 'CREATOR', 'PLATFORM_ADMIN')),
	CONSTRAINT "users_uid_check" CHECK (("users"."bilibili_uid" is not null and "users"."bilibili_uid" ~ '^[1-9][0-9]{0,19}$') or ("users"."role" = 'PLATFORM_ADMIN' and "users"."bilibili_uid" is null)),
...
CREATE TABLE "gift_order_status_history" (
	...
	"reason" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "gift_order_status_history_from_check" CHECK ("gift_order_status_history"."from_status" is null or "gift_order_status_history"."from_status" in ('UNCLAIMED', 'SUBMITTED', 'SHIPPED', 'CANCELLED')),
	CONSTRAINT "gift_order_status_history_to_check" CHECK ("gift_order_status_history"."to_status" in ('UNCLAIMED', 'SUBMITTED', 'SHIPPED', 'CANCELLED'))
);
--> statement-breakpoint
CREATE TABLE "gift_orders" (
	...
	CONSTRAINT "gift_orders_status_check" CHECK ("gift_orders"."status" in ('UNCLAIMED', 'SUBMITTED', 'SHIPPED', 'CANCELLED')),
...
CREATE TABLE "identity_challenges" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "identity_challenges_purpose_check" CHECK ("identity_challenges"."purpose" in ('REGISTER', 'RECOVER')),
	CONSTRAINT "identity_challenges_status_check" CHECK ("identity_challenges"."status" in ('PENDING', 'VERIFIED', 'CONSUMED', 'EXPIRED', 'CANCELLED')),
	CONSTRAINT "identity_challenges_proof_check" CHECK ("identity_challenges"."status" not in ('VERIFIED', 'CONSUMED') or ("identity_challenges"."verified_uid" is not null and "identity_challenges"."verified_at" is not null and "identity_challenges"."event_id" is not null))
);
--> statement-breakpoint
...
CREATE TABLE "snapshot_runs" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "snapshot_runs_status_check" CHECK ("snapshot_runs"."status" in ('SCHEDULED', 'RUNNING', 'READY', 'FAILED', 'PENDING_APPROVAL', 'FINALIZED', 'REJECTED', 'CANCELLED'))
);
--> statement-breakpoint
...
ALTER TABLE "gift_orders" ADD CONSTRAINT "gift_orders_snapshot_member_id_snapshot_attempt_members_id_fk" FOREIGN KEY ("snapshot_member_id") REFERENCES "public"."snapshot_attempt_members"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "gift_orders" ADD CONSTRAINT "gift_orders_shipped_by_user_id_users_id_fk" FOREIGN KEY ("shipped_by_user_id") REFERENCES "public"."users"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "creators" ADD CONSTRAINT "creators_user_uid_fk" FOREIGN KEY ("user_id","bilibili_uid") REFERENCES "public"."users"("id","bilibili_uid") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "identity_challenges" ADD CONSTRAINT "identity_challenges_verification_room_id_verification_rooms_id_fk" FOREIGN KEY ("verification_room_id") REFERENCES "public"."verification_rooms"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "identity_challenges" ADD CONSTRAINT "identity_challenges_expected_user_id_users_id_fk" FOREIGN KEY ("expected_user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "users_bilibili_uid_unique" ON "users" USING btree ("bilibili_uid");--> statement-breakpoint
...
CREATE UNIQUE INDEX "identity_challenges_code_unique" ON "identity_challenges" USING btree ("code_digest");--> statement-breakpoint
CREATE UNIQUE INDEX "identity_challenges_event_unique" ON "identity_challenges" USING btree ("event_id");--> statement-breakpoint
CREATE INDEX "identity_challenges_owner_idx" ON "identity_challenges" USING btree ("owner_digest");--> statement-breakpoint
CREATE INDEX "identity_challenges_expiry_idx" ON "identity_challenges" USING btree ("expires_at");--> statement-breakpoint
...
CREATE INDEX "snapshot_attempt_members_attempt_position_uid_idx" ON "snapshot_attempt_members" USING btree ("snapshot_attempt_id","source_position","bili_uid");--> statement-breakpoint
...
CREATE INDEX "snapshot_runs_period_id_idx" ON "snapshot_runs" USING btree ("period_start","id");
--> statement-breakpoint
...
BEGIN
  ...
  END IF;
  IF NEW.capture_completed_at IS NOT NULL AND NEW.consistency_status = 'CONSISTENT' THEN
    IF NEW.punctuality IS NULL OR NEW.capture_started_at IS NULL
      OR NEW.normalized_total IS NULL OR NEW.normalized_total IS DISTINCT FROM NEW.declared_total
      OR NEW.normalized_total <> (SELECT count(*) FROM snapshot_attempt_members WHERE snapshot_attempt_id = NEW.id) THEN
      RAISE EXCEPTION 'consistent snapshot totals must match the sealed member collection';
    END IF;
  END IF;
	RETURN NEW;
END;
...
CREATE FUNCTION enforce_snapshot_collection_seal() RETURNS trigger AS $$
DECLARE completed_at timestamptz;
BEGIN
  SELECT capture_completed_at INTO completed_at FROM snapshot_attempts
    WHERE id = NEW.snapshot_attempt_id FOR UPDATE;
  IF NOT FOUND OR completed_at IS NOT NULL THEN
    RAISE EXCEPTION 'completed snapshot collections are sealed';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;--> statement-breakpoint
CREATE TRIGGER snapshot_attempt_members_sealed
  BEFORE INSERT ON snapshot_attempt_members
  FOR EACH ROW EXECUTE FUNCTION enforce_snapshot_collection_seal();--> statement-breakpoint
CREATE TRIGGER snapshot_pages_sealed
  BEFORE INSERT ON snapshot_pages
  FOR EACH ROW EXECUTE FUNCTION enforce_snapshot_collection_seal();--> statement-breakpoint

CREATE FUNCTION enforce_snapshot_accepted_attempt() RETURNS trigger AS $$
BEGIN
  IF NEW.status = 'FINALIZED' THEN
    IF NEW.finalized_at IS NULL OR NEW.accepted_attempt_id IS NULL OR NOT EXISTS (
      SELECT 1 FROM snapshot_attempts WHERE id = NEW.accepted_attempt_id
        AND snapshot_run_id = NEW.id AND consistency_status = 'CONSISTENT'
        AND capture_completed_at IS NOT NULL
        AND ((punctuality = 'ON_TIME' AND NEW.approved_at IS NULL AND NEW.approved_by IS NULL)
          OR (punctuality = 'LATE' AND NEW.approved_at IS NOT NULL AND NEW.approved_by IS NOT NULL))
    ) THEN
      RAISE EXCEPTION 'a finalized snapshot requires its own completed, accepted attempt';
    END IF;
  ELSIF NEW.accepted_attempt_id IS NOT NULL OR NEW.finalized_at IS NOT NULL
    OR NEW.approved_at IS NOT NULL OR NEW.approved_by IS NOT NULL THEN
    RAISE EXCEPTION 'only finalized snapshots may accept an attempt';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;--> statement-breakpoint
CREATE TRIGGER snapshot_runs_accepted_attempt
  BEFORE INSERT OR UPDATE ON snapshot_runs
  FOR EACH ROW EXECUTE FUNCTION enforce_snapshot_accepted_attempt();--> statement-breakpoint
...
-- Orders store actual operations; expiry is derived from the release window.
CREATE FUNCTION enforce_gift_order_lifecycle() RETURNS trigger AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN RAISE EXCEPTION 'gift orders cannot be deleted'; END IF;
  IF TG_OP = 'INSERT' THEN
    IF NEW.status <> 'UNCLAIMED' OR NEW.version <> 1 THEN
      RAISE EXCEPTION 'gift orders must start unclaimed';
    END IF;
  ELSE
    ...
    IF OLD.user_id IS NOT NULL AND (NEW.user_id IS DISTINCT FROM OLD.user_id
      ...
      RAISE EXCEPTION 'gift order claim is immutable';
    END IF;
    IF NEW.version <> OLD.version + 1 THEN RAISE EXCEPTION 'gift order version must increment exactly once'; END IF;
    IF NOT (
      (OLD.status = 'UNCLAIMED' AND NEW.status = 'SUBMITTED')
      OR (OLD.status = 'SUBMITTED' AND NEW.status IN ('SHIPPED', 'CANCELLED'))
      OR (OLD.status = 'SHIPPED' AND NEW.status = 'SHIPPED')
    ) THEN RAISE EXCEPTION 'invalid gift order state transition'; END IF;
    IF OLD.status = 'SHIPPED' AND (NEW.shipped_at IS DISTINCT FROM OLD.shipped_at
      OR NEW.shipped_by_user_id IS DISTINCT FROM OLD.shipped_by_user_id) THEN
      RAISE EXCEPTION 'the original shipping operation is immutable';
    END IF;
  END IF;
  IF NEW.status = 'UNCLAIMED' THEN
    IF NEW.user_id IS NOT NULL OR NEW.submitted_at IS NOT NULL THEN
      RAISE EXCEPTION 'unclaimed orders cannot have a claim';
    END IF;
  ELSIF NEW.user_id IS NULL OR NEW.submitted_at IS NULL THEN
    RAISE EXCEPTION 'claimed orders require a claimant and timestamp';
  END IF;
  IF NEW.status = 'SHIPPED' THEN
    IF NEW.shipped_at IS NULL OR NEW.shipped_by_user_id IS NULL
      OR NEW.carrier_name IS NULL OR length(trim(NEW.carrier_name)) NOT BETWEEN 1 AND 120
      OR NEW.tracking_number IS NULL OR length(trim(NEW.tracking_number)) NOT BETWEEN 1 AND 160 THEN
      RAISE EXCEPTION 'shipped orders require shipping information, actor and timestamp';
    END IF;
  ELSIF NEW.shipped_at IS NOT NULL OR NEW.shipped_by_user_id IS NOT NULL
    OR NEW.carrier_name IS NOT NULL OR NEW.tracking_number IS NOT NULL THEN
    RAISE EXCEPTION 'only shipped orders can have shipping information';
  END IF;
  IF NEW.status = 'CANCELLED' THEN
    IF NEW.cancelled_at IS NULL OR NEW.cancel_reason IS NULL THEN
      RAISE EXCEPTION 'cancelled orders require a timestamp and reason';
    END IF;
  ELSIF NEW.cancelled_at IS NOT NULL OR NEW.cancel_reason IS NOT NULL THEN
    RAISE EXCEPTION 'only cancelled orders can have a cancellation';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;--> statement-breakpoint
CREATE TRIGGER gift_orders_lifecycle
  BEFORE INSERT OR UPDATE OR DELETE ON gift_orders
...
--> statement-breakpoint
CREATE FUNCTION preserve_account_identity() RETURNS trigger AS $$
BEGIN
  IF NEW.id IS DISTINCT FROM OLD.id OR NEW.username IS DISTINCT FROM OLD.username
    OR NEW.bilibili_uid IS DISTINCT FROM OLD.bilibili_uid THEN
...
$$ LANGUAGE plpgsql;
--> statement-breakpoint
CREATE TRIGGER users_preserve_identity BEFORE UPDATE ON users
